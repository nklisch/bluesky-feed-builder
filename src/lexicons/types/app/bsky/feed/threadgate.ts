...
export interface Record {
  ...
  post: string;
  allow?: (MentionRule | FollowingRule | ListRule | { $type: string; [k: string]: unknown })[];
  createdAt: string;
...
export function isRecord(v: unknown): v is Record {
  return isObj(v) && hasProp(v, "$type") && (v.$type === "app.bsky.feed.threadgate#main" || v.$type === "app.bsky.feed.threadgate");
}
...
export function isMentionRule(v: unknown): v is MentionRule {
  return isObj(v) && hasProp(v, "$type") && v.$type === "app.bsky.feed.threadgate#mentionRule";
}
...
export function isFollowingRule(v: unknown): v is FollowingRule {
  return isObj(v) && hasProp(v, "$type") && v.$type === "app.bsky.feed.threadgate#followingRule";
}
...
export function isListRule(v: unknown): v is ListRule {
  return isObj(v) && hasProp(v, "$type") && v.$type === "app.bsky.feed.threadgate#listRule";
}
