...
export function timestampFromNow(props?: TimeStampProps) {
  ...
  if (unit === "datetime") {
    return direction === "past"
      ? sql`now() at time zone ${Timezones[timezone]} - ${interval(i)}`.inlineParams()
      : sql`now() at time zone ${Timezones[timezone]} - ${interval(i)}`.inlineParams();
  }
  return sql`extract(epoch from now() at time zone ${Timezones[timezone]}) * ${calculateTimeUnitMultiplier("second", unit)}`.inlineParams();
}
...
export function calculateTimeUnitMultiplier(startingUnit: SmallTimeUnit, endingUnit: SmallTimeUnit) {
  if (startingUnit === endingUnit) return 1;
  const increasing = step[startingUnit] < step[endingUnit];
  const [start, end] = increasing ? [step[startingUnit], step[endingUnit]] : [step[startingUnit], step[endingUnit] - 1];
